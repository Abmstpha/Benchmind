...
export const AIConsultant: React.FC<AIConsultantProps> = () => {
  ...
  const getAIRecommendation = async () => {
    ...
    try {
      // NOTE: The backend endpoint typically expects a RunRequest schema
      // which includes 'project_name', 'task_description', and 'selected_models'.
      // You should adjust the API call structure in benchmindApi.getAIRecommendation
      // to match the backend's POST /api/run endpoint.
      
      // Assuming a temporary project name or defaulting to the task description
      const projectName = taskDescription.trim().substring(0, 50);

      const response = await benchmindApi.startRun({ // Renamed function for clarity
        project_name: projectName, 
        task_description: taskDescription.trim(),
        selected_models: selectedModels,
        // The userContext can be mapped to constraints or assumptions if needed.
        // For simplicity, passing empty constraints/assumptions here:
        constraints: {}, 
        assumptions: {} 
      });

      // Since startRun returns run_id, you need a mechanism to fetch results (SSE or polling)
      // For this clean file, we'll assume the API has a temporary endpoint
      // that does the sync consulting for now, or this component needs further integration 
      // with the SSE stream.

      // For now, let's assume the API function is the original one
      // If the original `getAIRecommendation` was actually calling POST /api/run, 
      // we need a follow-up call to GET /api/run/{run_id}/recommendation 
      
      // *** WARNING: This section needs robust SSE/polling logic for the real async workflow ***
      // For this clean file, we will re-use the synchronous call for simplicity,
      // but note the real solution involves streaming/polling on `response.run_id`.
      
      // Reverting to the original function name and assuming it handles the full flow for now
      const fullResponse = await benchmindApi.getAIRecommendation({
        task_description: taskDescription.trim(),
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        {recommendation && (
          <div className="mt-0 pt-0"> {/* Adjusted mt-6 border-t pt-6 to mt-0 pt-0 */}
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => {
                  const text = formatRecommendation(recommendation); // Use formatted recommendation
